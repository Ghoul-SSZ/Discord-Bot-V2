const fs = require("fs");
var heroes = require("../data/owheroes.json");
//TODO Make sure the generator doesn't choose duplicated
//(happnened duting the balanced search)
exports.run = (client, message, args) => {
	const action = args[0];
	if(action == "single" || action == null){
			...
			if(args[1] == "balanced" && players.length >= 4){
				switch(players.length){ //How many heroes do we need to generate
					...
					case 4:
						...
						break;
					default: //Probably dead code
						message.channel.send("Need at least 4 people in the voicechannel!");
						break;
				}
				...
				players.forEach(function(current){
					...
					message.channel.send(hero);
															...
															// TODO Merge offense and defense & add new hero 
...
function generateRandomGroupHero(user, permutationArray, type_num = getRandomInt(0, 3)){
	...
	let index = getRandomInt(0, heroArray.length - 1);

	let hero = permutationArray[type_num][index];
	permutationArray[type_num].splice(index, 1);

	return user + heroes[type_num][hero];
